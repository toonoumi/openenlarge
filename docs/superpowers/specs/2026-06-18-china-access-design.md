...
**Date:** 2026-06-18
**Status:** Phase 1 BUILT & live (see as-built note)

> **As-built (differs from original design below):** front door is **EdgeOne** on `openenlarge.io` (grey-cloud CNAME → `openenlarge.io.eo.dnse5.com`), origin = `openenlarge.pages.dev` deployed via **direct-upload CI** (`deploy-web.yml` + wrangler), not git-connected Pages. Downloads serve from R2 at **`download.aako.world/<tag>/…`** (not `dl.openenlarge.io`). `releases.js` reads same-origin `releases.json` generated by `release.yml`'s `publish-manifest` job. Phase 2 (in-app updater mirror) not done.
