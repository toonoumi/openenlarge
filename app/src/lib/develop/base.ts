...
/** Resolve the effective base for a frame and inject it into a throwaway
 * params object: per-image override -> folder default -> null (backend falls back).
 * Never mutates the persisted per-image base_override. The per-image
 * d_max_override already rides on `params`; D_max is scene-dependent so there is
 * no roll/folder default for it. */
